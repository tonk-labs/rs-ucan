...
impl<C: TryFrom<u64>> TryFrom<&[u8]> for EdDsaHeader<C> {
    type Error = (); // FIXME

    fn try_from(bytes: &[u8]) -> Result<Self, Self::Error> {
        if let Ok((0xed, inner)) = unsigned_varint::decode::u8(&bytes) {
            if let Ok((codec_info, &[])) = unsigned_varint::decode::u64(&inner) {
...
impl<C: Into<u64> + Clone> From<EdDsaHeader<C>> for Vec<u8> {
    fn from(ed: EdDsaHeader<C>) -> Vec<u8> {
        ...
        let enc: &[u8] = unsigned_varint::encode::u64(ed.codec.into(), &mut enc_buf);
